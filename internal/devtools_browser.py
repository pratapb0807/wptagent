...
class DevtoolsBrowser(object):
    ...
    def prepare_browser(self, task):
        ...
        if self.devtools is not None:
            ...
            try:
                self.devtools.send_command('Network.emulateNetworkConditions', { 'offline': False, 'downloadThroughput': task['bwIn'], 'uploadThroughput': task['bwOut'], 'latency': task['latency']})
    ...
    def run_lighthouse_test(self, task):
        ...
        if 'url' in self.job and self.job['url'] is not None:
            ...
            command = ['lighthouse',
                       ...
                       '--max-wait-for-load', str(int(time_limit * 1000 * 3)),
                       '--port', str(task['port']),
